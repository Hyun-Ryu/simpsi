...
class Freqdropout(nn.Module):
    ...
    def forward(self, x_t):
        # x_t: (b, 1, timestep) -> cplx
        mask = torch.rand(x_t.shape[0],1,1).to(x_t.device) < self.p

        x_f = torch.fft.fft(x_t, dim=-1)    # rev from rfft on 0811
        ...
        x_t_aug = torch.fft.ifft(x_f_aug, dim=-1, n=x_t.shape[-1])   # rev from irfft on 0811
        return x_t*(~mask) + x_t_aug*mask


class Dropout(nn.Module):
    def __init__(self, p_zero=0.5):
        super().__init__()
...
class AugBoostDeep(nn.Module):
    ...
    def forward(self, x_t, eq, y, model_f, model):
        # x_t: (b, 1, timestep) -> cplx
        # x_f: (b, 1, timestep) -> cplx
        x_t_org = x_t.clone().detach()
        x_f = torch.fft.fft(x_t)
        x_t = torch.cat((x_t.real, x_t.imag), dim=1) # x_t: (b, 2, timestep)
        x_f_mag = x_f.abs()
        x_f = torch.cat((x_f.real, x_f.imag), dim=1) # x_f: (b, 2, timestep)
        
        # Aug
        for aug in self.aug_list:
            aug_ = eval(aug.capitalize())()
            x_t = aug_(x_t)
        x_t_aug = x_t   # (b, 2, timestep)

        if self.prior == 'none':
            return x_t_aug, None, None, None
        
        # x_t_aug -> x_f_aug
        x_t_aug = x_t_aug[:, :1, :] + torch.tensor(1.j)*x_t_aug[:, 1:, :] # (b, 1, timestep) -> cplx
        x_f_aug = torch.fft.fft(x_t_aug)
        x_f_aug = torch.cat((x_f_aug.real, x_f_aug.imag), dim=1)          # (b, 2, timestep)

        # Boost the prior
        if self.prior == 'mag':
            boost_ = eq(x_f_mag)                    # x_f_mag: (b, 1, timestep)
        elif self.prior == 'slc':
            # stop gradient
            ...
            x_f_ = torch.fft.fft(x_t_org).clone().detach()      # x_f_: (b, 1, timestep) -> cplx
            x_f_ = torch.cat((x_f_.real, x_f_.imag), dim=1)     # x_f_: (b, 2, timestep) -> re
            x_f_.requires_grad = True
            x_f_cplx = x_f_[:, :1, :] + torch.tensor(1.j)*x_f_[:, 1:, :]
            x_t_cplx = torch.fft.ifft(x_f_cplx, n=x_t_org.shape[-1])
            preds = model(torch.cat((x_t_cplx.real, x_t_cplx.imag), dim=1).unsqueeze(2).float())
            score = torch.zeros(y.shape).cuda()
            rows = torch.arange(y.size(0)).unsqueeze(1)  # shape [64, 1]
            cols = y                                     # shape [64, 32]
            lns = torch.arange(y.size(-1)).unsqueeze(0)  # shape [1, 32]
            score[rows, lns] = preds[rows, cols, lns]
            score.mean().backward()
            ...
            eq = eq.double()
            boost_ = eq(x_f.double())            # x_f: (b, 2, timestep)
        elif self.prior == 'rnd':
        ...
        x_t_aug_psi = torch.fft.ifft(x_f_aug_psi, n=x_t.shape[-1])
        x_t_aug_rand = torch.fft.ifft(x_f_aug_rand, n=x_t.shape[-1])
        x_t_aug_lsi = torch.fft.ifft(x_f_aug_lsi, n=x_t.shape[-1])

        x_t_aug_psi = torch.cat((x_t_aug_psi.real, x_t_aug_psi.imag), dim=1)
        x_t_aug_rand = torch.cat((x_t_aug_rand.real, x_t_aug_rand.imag), dim=1)
        x_t_aug_lsi = torch.cat((x_t_aug_lsi.real, x_t_aug_lsi.imag), dim=1)

        return x_t_aug_psi.float(), x_t_aug_rand.float(), x_t_aug_lsi.float(), boost_
